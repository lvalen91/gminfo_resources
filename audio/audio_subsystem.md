...
Android **CarAudioService exposes exactly 8 output buses** (bus0–bus7), confirmed in `car_service.txt`. The **bus8–bus13** entries below are **Harman-HAL-internal routing-graph nodes** (from `AudioRoutingConfigParser`/`GraphManager`), NOT CarAudioService output buses — apps cannot target them via `CarAudioManager`:
