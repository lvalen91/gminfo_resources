...
- **ELK trigger chain**: `kf4abl.c` is the direct upstream source for understanding the ELK trigger sequence. Cross-reference with GM's `OBBPELK` symbol → `BootELKSendAblUserCommand` (address `0x0087452a`) → `ConnToOTA_BootELK` call graph recovered from gminfo37 decompilation. **Identity corrected (2026-09-10):** ELK is Intel Kernelflinger itself (`kernelflinger-07.02` / `titan_gm_my22`) — a `fastboot`-class bootloader (`flash/erase/getvar`, `oem set-storage`, AVB-active, unlock device-state-gated), **not** "Enter Linux Kernel" and not a Linux shell/USB-rootfs environment. See HARDWARE_HYPERVISOR_ATTACK_VECTORS.md §"RH850 VIP JTAG"/UART section for the full correction and the GWMv2-sibling caveat (GitHub issue #1, @DymOK93).
- **ELK trigger LOCATED (2026-09-10, HIGH confidence):** fresh VIP RH850 + GHS INTEGRITY disassembly finds the VIP-side handler is an unauthenticated internal serial-IPC message — `SERIAL_IPC_CONTROL_CHANNEL` ("Channel-7"), `RID=0xFF01` (big-endian) + `subcommand=0x01`, VIP handler `FUN_000641c2` @0x64208, no `$27`/session/dev/EEPROM gate at the VIP. `BootELKSendAblUserCommand` (this file's SoC-side symbol, above) is the HECI-to-CSE/ABL half of the same overall chain, not the VIP handler itself. GM does not use GWM's UDS DID `0xFDF0` for this trigger — see GitHub issue #1 (GWMv2 sibling, @DymOK93) for the DID that sibling platform uses instead. OPEN: whether an external DoIP/MDI2 tester or the uid-2000 Android guest can make the SoC emit 0xFF01 on Channel-7 — full detail and priority in HARDWARE_HYPERVISOR_ATTACK_VECTORS.md.
