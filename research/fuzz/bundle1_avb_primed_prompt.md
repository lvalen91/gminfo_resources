...
1. GROUND TRUTH (use freely): the `AvbVBMetaImageHeader` (256B) and `AvbDescriptor`
   field/offset tables, the Android boot-image header offsets, and the A/B misc struct
   below. `rollback_index` @ 0x70 is CONFIRMED. The parser IS stock AOSP `libavb`,
...
3. DO NOT invent disassembly, addresses, or function offsets. The addresses
   ...
   non-reproducible — never cite or build on them. `vmm1_all.c` is register noise; the
   only trustworthy things from it are the `0x200` (×512) literals at lines 4558/4574.
4. Every struct offset you emit MUST come from a table in the artifact below. If you
   need a value not given, write `ASSUMPTION: <what/why>` inline — do not guess silently.
...
<artifact name="VMM1_PARSER_FUZZ_TARGETS_ANALYSIS.md" provenance="analyst, binary-artifact-derived">
...
<artifact name="scripts/avb_audit.py" provenance="ground-truth: real Ghidra post-script; its marker list = the exact libavb error strings present in the binary">
```python
...
1. `avb_bundle1_fuzzer.c` — a libFuzzer harness (`LLVMFuzzerTestOneInput`) that drives
   upstream `libavb` built `-m32`: call `avb_vbmeta_image_verify()` on the input, and on
   accept, walk descriptors via `avb_descriptor_foreach()` and run the boot-image bound
   checks. Target the O1/O2 offset+len 64-bit-add-on-32-bit paths, O3 descriptor inner
   lengths, O5 boot-image `kernel_size`/`ramdisk_size`/`page_size`. Compile-ready; include
   the exact libavb headers/entrypoints you call.

2. `gen_seeds.py` — a seed-corpus generator that emits raw vbmeta blobs built STRICTLY
   from the header/descriptor offset tables in the artifact, implementing the §3 mutation
   set in order (high-dword-first offset/size pairs; descriptors_size vs
   num_bytes_following mismatch; non-%8; inner-length sum-wrap; boot-hdr page_size/kernel/
   ramdisk edges; misc CRC32 re-selection; rollback high-dword). All multi-byte fields
   big-endian. One file per mutation, named by target (O1_desc_off_highdword.bin, ...).

3. `build.sh` — clone/checkout upstream `libavb` at the **1.2** tag, build `-m32`, compile
   the harness with `-fsanitize=address,fuzzer`, and run against the generated corpus.

Rules: no offset appears that isn't in the artifact tables; annotate every O#/T# you
target; put `ASSUMPTION:`/`hardware-only` markers where the trust_filter requires.
Start with `gen_seeds.py` if it helps you fix the byte layout before the harness.
</task>
